...
function Test-Phase0MultiSourceIdleRetirementEvidence($Scenario) {
    if ([string]$Scenario.name -ne 'multi_source_pressure_and_idle_retirement' -or [int]$Scenario.iterations -ne 16) {
    ...
    if ($values.source_count -ne 12 -or $values.batch_count -ne 3 -or $values.lanes_per_batch -ne 4 -or $values.frame_bytes -ne 57600 -or $values.cache_cap_bytes -ne 172800 -or $values.cache_current_bytes -ne $values.cache_cap_bytes -or $values.cache_peak_bytes -ne $values.cache_cap_bytes -or $values.cache_eviction_count -lt 9 -or $values.peak_sessions -ne 4 -or $values.session_cap -ne 4 -or $values.peak_source_groups -ne 4 -or $values.source_group_cap -ne 4 -or $values.peak_lane_actors -ne 4 -or $values.lane_actor_cap -lt 4 -or $values.idle_release_cycles -ne 3 -or $values.idle_lru_reclaims -ne 1 -or $values.idle_lru_final_sessions -ne 0 -or $values.idle_lru_final_groups -ne 0 -or $values.idle_lru_final_live_actors -ne 0 -or $values.idle_lru_final_retiring_actors -ne 0 -or $values.final_sessions -ne 0 -or $values.final_source_groups -ne 0 -or $values.final_live_lane_actors -ne 0 -or $values.final_retiring_lane_actors -ne 0) {
